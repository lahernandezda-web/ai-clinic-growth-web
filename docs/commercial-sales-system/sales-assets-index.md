...
```mermaid
flowchart LR
  A[Lead] --> B[Cualificación]
  B --> C{Decisión}
  C -->|Nutrir| D[follow-up-sequences]
  C -->|Auditoría| E[free-audit-methodology]
  E --> F[audit-report-template]
  F --> G[mvp-proposal-template]
  G --> H[client-onboarding-checklist]
  H --> I[project-kickoff-template]
  I --> J[scope-control-playbook]
```
